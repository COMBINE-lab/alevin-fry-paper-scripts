---
title: "Clustering analysis for starsolo result of a mouse placenta dataset"
author: "Dongze He"
date: "6/9/2021"
...
indir = file.path(topdir, "results", "star_solo", "mouse_placenta", "star_solo", "Solo.out", "GeneFull", "raw")
...
# load raw matrix
```{r}
star_counts = Read10X(file.path(indir))
```

```{r}
source(emptyDrops_CR_path)

e.out = cellRangerLikeEmptyDrops(m=star_counts)
sum(e.out$is.cell)
```


```{r}
star_counts = star_counts[, e.out$is.cell]
```



```{r}
object <- CreateSeuratObject(counts = star_counts)
...
```{r}
pdf(file = file.path(outdir, "st_empryDropCR_filtered_QC.pdf"),   # The directory you want to save the file in
...
# further filtered by unique gene and mt percent
```{r}
...
```{r}
pdf(file = file.path(outdir, "st_empryDropCR_filtered_nuclei_QC.pdf"),   # The directory you want to save the file in
    width = 12, # The width of the plot in inches
    height = 8) # The height of the plot in inches
...
```{r}
# Step 1: Call the pdf command to start the plot
pdf(file = file.path(outdir, "st_empryDropCR_filtered_tsne.pdf"),   # The directory you want to save the file in
    width = 12, # The width of the plot in inches
    height = 8) # The height of the plot in inches
...
```{r}
# Step 1: Call the pdf command to start the plot
pdf(file = file.path(outdir, "st_empryDropCR_filtered_trophoblast_tsne.pdf"),   # The directory you want to save the file in
    width = 12, # The width of the plot in inches
    height = 8) # The height of the plot in inches
...
DotPlot(object = troph_object, features = markers, dot.scale = 3)  + theme(axis.text.x = element_text(angle = 50, hjust = 1, vjust = 1),axis.text.y = element_text(size = 20), axis.title.y = element_blank(), axis.title.x = element_blank())

```

```{r}
# Step 1: Call the pdf command to start the plot
pdf(file = file.path(outdir, "st_empryDropCR_filtered_trophoblast_dot.pdf"),   # The directory you want to save the file in
    width = 12, # The width of the plot in inches
    height = 8) # The height of the plot in inches
