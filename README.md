...
First of all, edit the config file `configs/config.json` to include the relavent paths for `kb`, `starsolo`, `salmon` and `alevin-fry` binaries. Also, set the `top_dir` in the config file to the address of the directory where you want to save all the files generated by these scritps.
