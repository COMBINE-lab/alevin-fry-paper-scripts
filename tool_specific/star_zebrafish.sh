...
cmd="/usr/bin/time -v -o $output/logs/solo.time $star --genomeDir $index_dir --outFileNamePrefix $output --soloType CB_UMI_Simple --soloUMIlen 10 --soloCBlen 16 --readFilesIn $reads --readFilesCommand zcat --soloCBwhitelist $permitlist --limitIObufferSize 50000000 50000000 --outSJtype None --outSAMtype None --runThreadN $threads --soloCBmatchWLtype 1MM_multi_Nbase_pseudocounts --soloUMIdedup 1MM_Directional --soloCellFilter EmptyDrops_CR --clipAdapterType CellRanger4 --outFilterScoreMin 30 --soloFeatures Gene"
echo $cmd
eval $cmd
