...
t2g=$top_dir/refs/refdata-cellranger-GRCh38-3.0.0/transcriptome_splici/transcriptome_splici_fl86_t2g.tsv
t2g_3cols=$top_dir/refs/refdata-cellranger-GRCh38-3.0.0/transcriptome_splici/transcriptome_splici_fl86_t2g_3col.tsv
...
mkdir -p $results_dir/alevin_fry
mkdir -p $results_dir/alevin_fry/sim_data
mkdir -p $results_dir/alevin_fry/fry_sla_unfilt_quant_usa_cr-like
mkdir -p $results_dir/alevin_fry/fry_sla_knee_quant_usa_cr-like
...
cmd="/usr/bin/time -v -o $output/logs/pseudoalignment.time $salmon alevin -l ISR \
        -i $txomic_index -1 $read1 -2 $read2 -o $output -p 16 --chromiumV3  --rad --sketch"
echo $cmd
eval $cmd
