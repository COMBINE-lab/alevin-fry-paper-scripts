...
cmd="/usr/bin/time -v -o $output/logs/pseudoalignment.time $salmon alevin -l ISR \
        -i $splici_index -1 $reads1 -2 $reads2 -o $output -p 16 --chromiumV3  --rad"
echo $cmd
eval $cmd

./run_alevinfry.sh $fry $output $t2g_3cols $threads knee
./run_alevinfry.sh $fry $output $t2g_3cols $threads unfilt $unf_list
...
./run_alevinfry.sh $fry $output $t2g $threads knee
./run_alevinfry.sh $fry $output $t2g $threads unfilt $unf_list
